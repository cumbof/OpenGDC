...
public class MetadataParserXML extends BioParser {
	...
	public int convert(String program, String disease, String dataType, String inPath, String outPath) {
		...
		String FIXED_PROGRAM = "TCGA";
		String FTP_BED_ROOT_PATH = "/FTP/ftp-root/opengdc/bed/"+FIXED_PROGRAM.toLowerCase()+"/";

		HashMap<String, String> dataTypes_formatted = GDCData.getGDCData2FTPFolderName();
		HashMap<String, HashSet<String>> tcga_data_map = GDCData.getBigGDCDataMap().get(FIXED_PROGRAM);

		HashMap<String, ArrayList<String>> aliquot2ogdcids = new HashMap<>();
		for (String expDataType: tcga_data_map.get(disease)) {
			if (!expDataType.toLowerCase().contains("clinical") && !expDataType.toLowerCase().contains("biospecimen")) {
				String bed_dir_path = FTP_BED_ROOT_PATH + disease.toLowerCase() + "/" + dataTypes_formatted.get(expDataType.toLowerCase()).toLowerCase() + "/";

				for (File converted_file: (new File(bed_dir_path)).listFiles()) {
					if (converted_file.getName().toLowerCase().endsWith("bed")) {
						String[] converted_file_name_arr = converted_file.getName().split("-");
						String converted_file_name = converted_file_name_arr[0];
						for (int i=1; i<converted_file_name_arr.length-1; i++)
							converted_file_name = converted_file_name + "-" + converted_file_name_arr[i];
						String opengdcSuffix = converted_file_name_arr[converted_file_name_arr.length-1].split("\\.")[0];

						ArrayList<String> opengdcids = new ArrayList<>();
						opengdcids.add(opengdcSuffix);
						if (aliquot2ogdcids.containsKey(converted_file_name))
							opengdcids.addAll( aliquot2ogdcids.get(converted_file_name) );
						aliquot2ogdcids.put(converted_file_name, opengdcids);
					}
				}
			}
		}

		HashSet<String> todo_aliquots = new HashSet<>();
		for (String aliquotid: aliquot2ogdcids.keySet()) {
			boolean all_ok = true;
			for (String ogdcid: aliquot2ogdcids.get(aliquotid)) {
				String meta_file_path = FTP_BED_ROOT_PATH + disease.toLowerCase() + "/" + "clinical_and_biospecimen_supplements" + "/" + aliquotid + "-" + ogdcid + ".meta";
				if (!(new File(meta_file_path)).exists()) {
					all_ok = false;
					break;
				}
			}
			if (!all_ok){
				String aliquotUpper = aliquotid.toUpperCase();
				todo_aliquots.add(aliquotUpper);
			}
		}
					...
					if (f.getName().toLowerCase().contains("clinical")) {                        
						...
						for (XMLNode aliquot: MetadataHandler.getAliquotNodes()) {
							...
							if (!aliquot_uuid.equals("")) {
								...
								HashMap<String, String> parentMeta = MetadataHandler.extractParentMetadata(aliquot.getParent(), new HashMap<String,String>());
								HashMap<String, String> parentMetanoadmin = new HashMap<String, String>();
                                parentMetanoadmin.putAll(parentMeta);
						...
						if (!aliquot2attributes.isEmpty()) {
							for (String aliquot_uuid: aliquot2attributes.keySet()) {
								if(todo_aliquots.contains(aliquot_uuid.toLowerCase()) || todo_aliquots.contains(aliquot_uuid.toUpperCase())){
		...
		System.err.println("todo_aliquots.size : "+todo_aliquots.size());
		System.err.println("biospecimenBigMap.size : "+biospecimenBigMap.size());

		ArrayList<String> skippedAliquots = convertProcedure(program, disease, dataType, outPath, clinicalBigMap, biospecimenBigMap, new ArrayList<String>(), aliquotUUID2fileUUID);
